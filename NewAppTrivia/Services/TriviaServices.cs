...
namespace NewAppTrivia.Services
{
    public class TriviaServices
    {
        ...
        {
             string json = JsonSerializer.Serialize(new { email = playerName, password = password },options);
             StringContent content = new StringContent(json, Encoding.UTF8, "application/json");
            var response = await httpClient.PostAsync($"{URL}Login", content);

            if (response.StatusCode == System.Net.HttpStatusCode.OK)
                return true;
            else if (response.StatusCode == System.Net.HttpStatusCode.BadRequest|| response.StatusCode == HttpStatusCode.NotFound)
                return false;
            return false;
